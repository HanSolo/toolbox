...
public class VersionNumber implements Comparable<VersionNumber> {
    ...
    public static VersionNumber fromText(final String text, final int resultToMatch) throws IllegalArgumentException {
        if (null == text || text.isEmpty()) {
            System.out.println("No version number can be parsed because given text is null or empty.");
        ...
        if (numbersFound.isEmpty()) {
            System.out.println("No suitable version number found in String: " + text);
            return new VersionNumber();
        } else {
    ...
    private static Integer getLeadingIntFromText(final String text, final String fullTextToParse) {
        ...
        if (matcher.find()) {
            ...
            System.out.println("Given text " + text + " did not start with integer. Full text to parse was: " + fullTextToParse);
            return -1;
        }
    }
